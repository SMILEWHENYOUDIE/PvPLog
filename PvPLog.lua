...
function PvPLogOnEvent()   
   ...
   elseif (event == "PLAYER_ENTERING_WORLD") then
	 ...
	 softPL = true;
		...
		info.config = 0;
		info.faction = PvPLogGetFaction();
		if (info.faction == "Horde") then
			info.icon = "Interface\\Icons\\INV_BannerPvP_01";
		else
			info.icon = "Interface\\Icons\\INV_BannerPvP_02";
		end
...
function PvPLogInitialize()   
   ...
   PvPLogData[realm][player].vendor = VER_VENDOR;
	...
	if (getn(PvPLogData[realm][player].MiniMap) < 7) then
		PvPLogData[realm][player].MiniMap.position = -45; -- default only. after first use, SavedVariables used
		PvPLogData[realm][player].MiniMap.drag = "CIRCLE"; -- default only. after first use, SavedVariables used
		PvPLogData[realm][player].MiniMap.tooltip = UI_RIGHT_CLICK .. UI_TOGGLE;
		PvPLogData[realm][player].MiniMap.enabled = 1; -- default only. after first use, SavedVariables used
		PvPLogData[realm][player].MiniMap.config = 0;
		PvPLogData[realm][player].MiniMap.faction = PvPLogGetFaction();

		if (PvPLogData[realm][player].MiniMap.faction == "Horde") then
			PvPLogData[realm][player].MiniMap.icon = "Interface\\Icons\\INV_BannerPvP_01";
		else
			PvPLogData[realm][player].MiniMap.icon = "Interface\\Icons\\INV_BannerPvP_02";
		end
	end
...
function PvPLogSlashHandler(msg)
   ...
   PurgeLogData[realm][player].mouseover = true;
	 ...
	 PvPLogData[realm][player].version = VER_NUM;
	 PvPLogData[realm][player].vendor = VER_VENDOR;
	 PvPLogChatMsgCyan("PvPLog " .. MAGENTA .. RESET .. " " .. CYAN 
		       .. COMP);
      end
